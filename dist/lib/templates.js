 ...
 */
export function copyTemplates(targetDir) {
    ...
    for (const file of TEMPLATE_FILES) {
        ...
        if (!existsSync(src)) {
            if (file.required) {
                console.error(chalk.red(`  Error: Template file not found: ${file.name}`));
            }
            continue;
        }
 ...
 */
export function createInitialConfig(configDir, inst, defaultModel, globalConfig) {
 ...
 */
export function syncInstanceFolders(config, paths) {
    console.log(chalk.green("=== Syncing instance folders ==="));
    const defaultModel = getPath(config.config || {}, "agents.defaults.model.primary") || DEFAULTS.model;
